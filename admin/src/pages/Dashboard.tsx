...
const CORAL = '#F4451E', INK = '#09090B', PALETTE = ['#F4451E', '#09090B', '#71717A', '#FF9A7E', '#8FB5AC', '#C6D4CF', '#7A8C87', '#FFC9B8']
    ...
    <div>
      <div className="flex items-end justify-between flex-wrap gap-3">
        ...
        <div className="segmented">
          {[7, 30, 90].map(n => (
            <button key={n} className={days === n ? 'on' : ''} onClick={() => setDays(n)}>{n} days</button>
          ))}
        </div>
      </div>
      <div className="grid grid-cols-2 lg:grid-cols-5 gap-4 mt-6">
        {stats.map(s => (
          <div key={s.k} className="card p-5">
            <p className="eyebrow m-0" style={{ color: '#71717A' }}>{s.k}</p>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-4">
        <div className="card p-5 lg:col-span-2">
          <p className="eyebrow m-0 mb-4" style={{ color: '#71717A' }}>Visitors by day</p>
        ...
        <div className="card p-5">
          <p className="eyebrow m-0 mb-2" style={{ color: '#71717A' }}>Devices</p>
      ...
      <div className="grid lg:grid-cols-3 gap-4 mt-4">
        <div className="card p-5">
          <p className="eyebrow m-0 mb-4" style={{ color: '#71717A' }}>Top countries</p>
        ...
        <div className="card p-5">
          <p className="eyebrow m-0 mb-3" style={{ color: '#71717A' }}>Top pages</p>
          {d.topPages.map((p: any) => (
            <div key={p.k} className="flex justify-between text-sm py-1.5" style={{ borderBottom: '1px solid #E4E4E7' }}>
              <span className="truncate mr-3">{p.k}</span><span className="font-semibold">{p.c}</span>
            </div>
          ))}
        </div>
        <div className="card p-5">
          <p className="eyebrow m-0 mb-3" style={{ color: '#71717A' }}>Browsers & referrers</p>
          ...
          {d.topReferrers.slice(0, 5).map((p: any) => (
            <div key={p.k} className="flex justify-between text-sm py-1.5" style={{ borderBottom: '1px solid #E4E4E7', color: '#71717A' }}>
