  ...
  return (
    <div>
      <div className="flex items-end justify-between flex-wrap gap-3 mb-6">
        <div>
          <h1 className="page-title m-0">Blog posts</h1>
        </div>
      ...
      <div className="card overflow-x-auto">
        <table className="tbl">
          ...
          <tbody>
            {items.map(b => (
              <tr key={b.id}>
                <td><Link to={`/blogs/${b.id}`} className="font-semibold no-underline" style={{ color: '#09090B' }}>{b.title}</Link></td>
                <td style={{ color: '#71717A' }}>/{b.slug}</td>
                <td>{b.topic || '—'}</td>
                <td><span className="text-[11px] font-bold uppercase px-2 py-1 rounded-full" style={{ background: b.status === 'published' ? '#DCFCE7' : '#F4F4F5', color: b.status === 'published' ? '#16A34A' : '#71717A' }}>{b.status}</span></td>
                <td>{b.views}</td>
                <td>{b.source}</td>
                ...
                <td className="whitespace-nowrap">
                  ...
                  <button className="tt-btn" onClick={() => del(b)} style={{ color: '#DC2626' }}><Trash2 size={14} /></button>
                </td>
              </tr>
            ))}
            {!items.length && <tr><td colSpan={8} className="text-center py-10" style={{ color: '#71717A' }}>No posts yet — create your first one.</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
